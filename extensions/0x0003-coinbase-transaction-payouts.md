# Stratum V2 Extension: Coinbase Transaction Payouts

## 0. Abstract

This document defines a Stratum V2 extension enabling direct on-chain payout settlement under the Job Declaration Protocol (JDP): the coinbase transaction of a declared job pays multiple miners of the same Pool directly, according to the Pool's accounting window.

This removes the Pool's custody of funds for the miners represented in the payout distribution (typically the top miners of the Pool's accounting window), but it does not remove trust in the Pool's payout policy: the Pool still dictates payout scripts and weights, and a misbehaving Pool can publish a bad distribution. Funds of miners not represented in the distribution remain under Pool custody, exactly as under custodial payouts (see [§9.3](#93-trust-model)).
...
Base JDP cannot support direct multi-miner coinbase payouts due to an ordering constraint:
...
| additional_outputs  | SEQ0_64K[B0_64K] | Consensus-serialized tx outputs added by Pool. Amount fields encode fixed satoshi amounts, paid exactly as declared ([§4](#4-payout-computation)); MAY be 0 (e.g. `OP_RETURN`). |
...
| `F`              | `SetPayoutDistribution.additional_outputs` | sum of fixed output amounts (satoshis)                                                     |
| `R`              | computed                                   | revenue remaining for the proportional split: `T − F`                                      |
...
F         = Σ additional_outputs[j].value
R         = T − F
amount_P  = floor(weight_P · R / W)
amount[i] = floor(weight[i] · R / W)
pay[i]    = if amount[i] ≥ dust_limits[i] { amount[i] } else { dust-pruned }
pay_P     = T − Σ pay[i] − F
```

Fixed amounts are funded off the top (`R = T − F`), so Pool and miners co-fund `additional_outputs` in proportion to their weights. A Pool wishing to fund them from its own share alone can account for that when setting `weight_P`. Fixed-amount outputs are paid exactly as declared and are not subject to dust pruning; the Pool is responsible for declaring sane amounts.

If `F ≥ T`, the distribution is unsatisfiable for this template. This is a Pool misconfiguration of the same class as the blockspace violation in [§5](#5-coinbaseoutputconstraints): JDC SHOULD apply the fallback strategy of [JDP §6.2](../06-Job-Declaration-Protocol.md#62-job-declarator-client).
...
Intermediate arithmetic MUST use at least 128-bit integers to avoid potential overflows (`weight · R` can reach ~2^115, `W` can reach ~2^80).
...
3. `additional_outputs` (if any, in distribution order, amounts MUST match the fixed amounts declared in `SetPayoutDistribution`)
...
Any mismatch MUST be rejected with `invalid-payout-distribution`. This includes fixed-amount outputs that are reduced, omitted, or otherwise modified: there is no partial payment of `additional_outputs`. Unlike base [JDP §6.4.3](../06-Job-Declaration-Protocol.md#643-allocateminingjobtokensuccess-server---client) (Example B), where JDC may allocate less than the full template revenue to the pool payout output, this extension gives JDC no discretion over payout amounts.
...
The Pool still dictates scripts and weights, so the trust assumptions around payout policy are unchanged relative to custodial mining — settlement moves on-chain, but distribution remains Pool-dictated (hence the name of this extension). However, miners included in the distribution gain coinbase-level verifiability of their payout, and dust-pruned/rounded amounts are auditable (they flow to `pay_P` by construction).
