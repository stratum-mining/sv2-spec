...
The serialized size of `coinbase_tx_outputs` is what the JDC turns into the `coinbase_output_max_additional_size` it reserves with its Template Provider (§7.1). The Server SHOULD NOT change that serialized size between `AllocateMiningJobToken.Success` messages unless required by changes to the pool’s configuration, so the JDC does not have to re-reserve. Notably, if the space the pool requires for coinbase transaction outputs varies, the Server SHOULD size `coinbase_tx_outputs` — padding with `0`-value outputs as needed — to the maximum it anticipates, so the reservation can stay fixed.
